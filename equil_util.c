...
double interpolate(double *xp, double *yp, double xb)
{
  ...
  if (print_flag == 1) {
    for (i=0; i<4; i++) {
      printf("interpolate: i=%d, xp = %.5e, yp = %f \n", i, xp[i], yp[i]); 
    }
    printf("interpolate: xb = %.10f, yb = %f \n", xb, yb);
  }
...
/**************************************************************************
 * interpolate_matrix is used to find gama, rho at the surface of the star.
 *************************************************************************/

double interpolate_matrix(double *xp, double **yp, double xb)
{
  int print_flag = 0,i;
  double yb;

  yb= (xb-xp[1])*(xb-xp[2])*(xb-xp[3])*yp[0][1]/
        ((xp[0]-xp[1])*(xp[0]-xp[2])*(xp[0]-xp[3]))

    + (xb-xp[0])*(xb-xp[2])*(xb-xp[3])*yp[1][1]/
       ((xp[1]-xp[0])*(xp[1]-xp[2])*(xp[1]-xp[3]))

    + (xb-xp[0])*(xb-xp[1])*(xb-xp[3])*yp[2][1]/
       ((xp[2]-xp[0])*(xp[2]-xp[1])*(xp[2]-xp[3]))

    + (xb-xp[0])*(xb-xp[1])*(xb-xp[2])*yp[3][1]/
       ((xp[3]-xp[0])*(xp[3]-xp[1])*(xp[3]-xp[2]));

  //printf("flag = %d/n", print_flag);
  if (print_flag == 1) {
    for (i=0; i<4; i++) {
      printf("interpolate: i=%d, xp = %.5f, yp = %6.5e \n", i, xp[i], yp[i][1]); 
    }
    printf("interpolate: xb = %.10f, yb = %6.5ef \n", xb, yb);
  }
  return (yb);
}
